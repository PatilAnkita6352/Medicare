...
$pre = "SELECT * FROM prescription WHERE appointment_id = '$appointment_id' ";
                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            while ($docRow=mysqli_fetch_array($results)) {
                                ...
                                $pmname=$docRow['m_name'];
                                 ...
                                 $resultss=mysqli_query($conn,"SELECT * FROM medicines WHERE med_name='$pmname' ");
                             while ($Row=mysqli_fetch_array($resultss) ) {
                                $price=$Row['price'] * $docRow['quantity'];
                                echo "<td>" . $price. " /-RS</td>";
                                $sum=$sum+$price;
...
left: 460px;" >Done</button>
                                    ...
                                    <a href ="viewprescription.php"><button class='btn btn-danger' name='back' style="position: relative;
top: -42px;
...
$mname=$_SESSION['username']; 
    ...
    $done = mysqli_query($conn,"INSERT INTO medicalbill (appointment_id,pname,user_email,age,gender,contactno,aadharno,address,booking_date,hname,tmode,doc_name,total,mname) VALUES ('$app_id','$pname','$user_email','$age','$gender','$contactno','$aadharno','$address','$booking_date','$hname','$tmode','$doc_name','$ea','$mname');");
    if ($done) {
        echo "<script> alert('Bill Generated Successfully');
